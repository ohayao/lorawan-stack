...
func (gs *GatewayServer) handleUpstream(conn connectionEntry) {
	...
	for {
		...
		case msg := <-conn.Up():
			...
			fmt.Printf("@@@@@@ GSServer upstream conn.UP [%s]\n", string(bs))
			//mac_payload := msg.Payload.GetMacPayload()
			//b64d, err := base64.StdEncoding.DecodeString(string(mac_payload.FrmPayload))
			//fmt.Printf("@@@@@@ DevAddr:%s FPort:%d %v FrmPayload_b64d:%s\n", mac_payload.DevAddr, mac_payload.FPort, err, string(b64d))
			val = msg
