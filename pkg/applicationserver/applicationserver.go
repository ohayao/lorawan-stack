...
func (as *ApplicationServer) downlinkQueueOp(ctx context.Context, ids ttnpb.EndDeviceIdentifiers, items []*ttnpb.ApplicationDownlink, op func(ttnpb.AsNsClient, context.Context, *ttnpb.DownlinkQueueRequest, ...grpc.CallOption) (*pbtypes.Empty, error)) error {
	...
	go as.MyhookDownlink(ctx, ids, items, peer.Name())
...
func (as *ApplicationServer) Myhook(ctx context.Context, up *ttnpb.ApplicationUp, link *ttnpb.ApplicationLink) {
	defer func() {
		if r := recover(); r != nil {
			fmt.Println("applicationserver.go hook error", r)
		}
	}()
	bs, _ := json.Marshal(up)
	fmt.Println("applicationserver.go hook 1", string(bs))
	us := up.GetUplinkMessage()
	bs1, _ := json.Marshal(us)
	fmt.Println("applicationserver.go hook 2", string(bs1))
}

func (as *ApplicationServer) MyhookDownlink(ctx context.Context, ids ttnpb.EndDeviceIdentifiers, items []*ttnpb.ApplicationDownlink, peerName string) {
	defer func() {
		if r := recover(); r != nil {
			fmt.Println("applicationserver.go hookdownlink error", r)
		}
	}()
	for _, ad := range items {
		bs, _ := json.Marshal(ad)
		fmt.Println("applicationserver.go hookdownlink ", string(bs))
	}
}
