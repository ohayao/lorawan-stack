...
func (s *srv) handleUp(ctx context.Context, state *state, packet encoding.Packet) error {
	...
	case encoding.PushData:
		...
		pushDataMsg, _ := json.Marshal(msg)
		fmt.Println("PushData Msg", string(pushDataMsg))
...
func (s *srv) myHook(ctx context.Context, packet encoding.Packet) {
	packetStr, _ := json.Marshal(packet)
	fmt.Printf("@@@@@@ [%s][%s] [%s]\n", packet.ReceivedAt.Format("2006-01-02 15:04:05"), packet.PacketType.String(), string(packetStr))
	if packet.PacketType == encoding.PushData && 1 > 2 {
		fmt.Printf("HOOK IN [%s][%s] ============>>>>>>\n", packet.ReceivedAt.Format("2006-01-02 15:04:05"), packet.PacketType.String())
		packetStr, _ := json.Marshal(packet)
		fmt.Println("packetStr: ", string(packetStr))
		p := encoding.Packet{}
		p.PacketType = encoding.PullResp
		p.GatewayEUI = packet.GatewayEUI
		p.GatewayAddr = packet.GatewayAddr
		...
		ps, _ := json.Marshal(p)
		fmt.Println("packet", p, string(ps))
		bs, _ := p.MarshalBinary()
		if l, err := s.conn.WriteToUDP(bs, packet.GatewayAddr); err != nil {
			fmt.Println("WriteToUDP Error ", err)
		} else {
			fmt.Printf("WriteTOUDP Success [%d]\n", l)
		}
	}
}
