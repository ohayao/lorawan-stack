...
func (s *impl) LinkGateway(link ttnpb.GtwGs_LinkGatewayServer) error {
	...
	for {
		...
		case down := <-conn.Down():
			...
			fmt.Println("@@@@@@ conn.Down", string(bs))
...
func (s *impl) Myhook(link ttnpb.GtwGs_LinkGatewayServer, up *ttnpb.GatewayUp) {
	defer func() {
		if r := recover(); r != nil {
			fmt.Println("hook panic", r)
		}
	}()
	if len(up.GetUplinkMessages()) > 0 {
		down := &ttnpb.DownlinkMessage{
			RawPayload: []byte{0x06, 0x06, 0x06, 0x06, 0x06, 0x06},
			EndDeviceIds: &ttnpb.EndDeviceIdentifiers{
				DevAddr:  &types.DevAddr{01, 14, 05, 34},
				DeviceId: "eui-e32ea31ec45fb738",
				DevEui:   &types.EUI64{0xE3, 0x2E, 0xA3, 0x1E, 0xC4, 0x5F, 0xB7, 0x38},
		...
		if err := link.SendMsg(msg); err != nil {
			fmt.Println("try send msg error", err, msg)
		} else {
			fmt.Println("try send msg success", msg)
		}
	}
}
